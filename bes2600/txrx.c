 ...
 * rx_highest 0x41.
 *
 * Tried, and it does not work on this hardware.  Built as a module
 * (CONFIG_WLAN_VENDOR_ST=y is needed as well -- CW1200 sits behind that gate
 * in drivers/net/wireless/st/Kconfig and is otherwise dropped silently), with
 * bes2600/factory.bin generated by tools/mkfactory.py, cw1200_wlan_sdio
 * probes and then fails:
 *
 *   cw1200_wlan_sdio: Probe called
 *   bes2600: rx-ready read failed: -110
 *   cw1200_wlan_sdio mmc2:0001:1: probe with driver cw1200_wlan_sdio failed
 *
 * It is the register read itself timing out, not the zero-buffer-count retry
 * that used to be broken here -- megi's bes_slave_rx_ready() already has the
 * "!ret && !*cnt" form this driver had to be fixed into.  The chip simply
 * stops answering partway through his bring-up, and it happens from a cold
 * boot with this driver blacklisted, so it is not contention between the two.
 * LuneOS's kernel recipe calls it the "WIP cw1200-based bring-up", which fits.
 *
 * Worth knowing for anyone who tries again: udev auto-loads cw1200 by
 * modalias as soon as the module exists, and whichever driver wins the race
 * leaves the other failing with -EBUSY and no wlan interface at all.  Both
 * need blacklisting against each other, not just one.
 *
 * So the control this would have provided is still missing.  The AX210
 * comparison ruled out the hardware and the channel; nothing has yet ruled
 * out this driver.
 */
